...
export function StatsCards() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
      ...
      <div className={`bg-white dark:bg-[#1A1A2E] rounded-xl shadow-md hover:shadow-lg transition-all duration-300 border-l-4 ${esMejor ? "border-[#10D47C]" : "border-[#FF5757]"} p-6 group cursor-pointer hover:-translate-y-1`}>
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs font-bold uppercase tracking-wider text-[#424C63] dark:text-[#B8BCC8]">
            vs Promedio
          </p>
          <div className={`h-12 w-12 ${esMejor ? "bg-gradient-to-br from-[#10D47C] to-[#2BE592]" : "bg-gradient-to-br from-[#FF5757] to-[#FF7777]"} rounded-full flex items-center justify-center shadow-md group-hover:scale-110 transition-transform`}>
            {esMejor ? (
        ...
        <div className="flex items-baseline gap-2 mb-1">
          <p className={`text-4xl font-bold ${esMejor ? "text-[#10D47C]" : "text-[#FF5757]"}`}>
            {comparacion > 0 ? "+" : ""}
